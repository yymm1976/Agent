...
import { decomposeWithSAD, type SADConfig } from './sad-decomposer.js';
import { BiEncoderSkillRetriever, type BiEncoderConfig } from './bi-encoder-retriever.js';
import { DecompositionGranularityAuditor, type GranularityAuditConfig } from './granularity-auditor.js';
import { CompatibilityScorer, type CompatibilityScorerConfig } from './compatibility-scorer.js';
import { SkillContextOptimizer, type SkillContextOptimizerConfig } from './context-optimizer.js';
...
/**
 * Phase 64：技能路由配置（与 src/config/schema.ts 中 skillRouting 对齐）
 *
 * 所有子开关默认 false，调用方需显式开启。包装函数 decomposeWithSADIfEnabled
 * 仅在 skillRoutingConfig.enabled = true 时才走新模块路径，否则降级到原有管线。
 *
 * 注意：SADConfig 自身不含 enabled 字段（仅 maxIterations/convergenceTau/inputSideFeedback），
 * 这里通过 intersection 类型补上 enabled，与 schema.ts 中 sad 对象的 shape 对齐。
 */
export interface SkillRoutingConfig {
  /** 总开关：false 时直接走原有 decomposeWithSkillAwareness 管线 */
  enabled?: boolean;
  /** SAD 不动点迭代分解配置（额外包含 enabled 开关） */
  sad?: SADConfig & { enabled?: boolean };
  /** Bi-encoder 语义检索配置 */
  biEncoder?: BiEncoderConfig;
  /** 分解粒度审计配置 */
  granularityAudit?: GranularityAuditConfig;
  /** 兼容性评分器配置 */
  compatibilityScorer?: CompatibilityScorerConfig;
  /** 上下文优化器配置 */
  contextOptimizer?: SkillContextOptimizerConfig;
}
...
/**
 * 把 SkillDAGPlan 中现有 edges 重建为 CompatibilityScorer 所需的候选边列表
 *
 * 内部辅助函数：从 plan.nodes 与 plan.edges 构造 {from, to, dependencyType} 数组，
 * 仅保留两端节点都存在的边。纯函数。
 */
function buildCandidateEdges(plan: SkillDAGPlan): Array<{
  from: SkillDAGNode;
  to: SkillDAGNode;
  ...
  for (const e of plan.edges) {
    ...
    if (from && to) {
      candidates.push({ from, to, dependencyType: e.dependencyType });
    }
  }
  return candidates;
}
...
/**
 * 走原有 decompose-retrieve-compose 管线（无任何 Phase 64 新模块）
 *
 * 当 skillRouting 未开启时使用。保留与 Phase 52 完全一致的行为，确保向后兼容。
 */
async function runOriginalPipeline(
  ...
  config: CompositionalRoutingConfig,
  decomposeFn: (task: string) => Promise<AtomicSubTask[]>,
): Promise<SkillDAGPlan> {
  const subTasks = await decomposeWithSkillAwareness(task, skills, config, decomposeFn);
...
/**
 * Phase 64 接线入口：SAD 包装函数
 *
 * 行为：
 *   - skillRoutingConfig 未提供或 enabled !== true → 走原有 decomposeWithSkillAwareness 管线
 *   - sad.enabled → 调用 decomposeWithSAD（input-side 反馈迭代分解）
 *   - granularityAudit.enabled → 对分解结果做粒度审计（仅记录日志，不强制重写）
 *   - compatibilityScorer.enabled → 用 CompatibilityScorer 过滤 DAG 边
 *   - contextOptimizer.enabled → 用 SkillContextOptimizer 构建优化上下文（需要 biEncoder 配合）
 *
 * 关于 retrieveFn 的类型约束：
 *   decomposeWithSAD 要求 retrieveFn 为同步函数，而 BiEncoderSkillRetriever.retrieve
 *   返回 Promise（embedder.embed 是 async）。因此 SAD 内部仍使用同步的 retrieveSkill，
 *   bi-encoder retriever 在 contextOptimizer 阶段（async 兼容）被实际调用。
 *
 * 所有新增调用受 skillRoutingConfig 各子开关守护，默认 false——不破坏现有功能。
 */
export async function decomposeWithSADIfEnabled(
  task: string,
  skills: Array<{ id: string; name: string; description: string; category: string }>,
  config: CompositionalRoutingConfig,
  decomposeFn: (task: string, hints?: string[]) => Promise<AtomicSubTask[]>,
  skillRoutingConfig?: SkillRoutingConfig,
): Promise<SkillDAGPlan> {
  // 总开关未开启 → 直接走原有管线
  if (!skillRoutingConfig || !skillRoutingConfig.enabled) {
    return runOriginalPipeline(task, skills, config, decomposeFn);
  }

  // 复用 bi-encoder retriever（sad 与 contextOptimizer 都可能需要）
  let biRetriever: BiEncoderSkillRetriever | null = null;
  if (skillRoutingConfig.biEncoder?.enabled) {
    biRetriever = new BiEncoderSkillRetriever(skillRoutingConfig.biEncoder);
    try {
      await biRetriever.initialize(skills);
    } catch (err) {
      logger.warn('compositional-router: BiEncoder 初始化失败，降级关键词检索', {
  ...
  // 1. 分解阶段
  let subTasks: AtomicSubTask[];
  if (skillRoutingConfig.sad?.enabled) {
    // SAD 的 retrieveFn 必须是同步函数；bi-encoder 的 async retrieve 无法直接传入。
    // 仍使用 retrieveSkill，但 bi-encoder retriever 会在后续 contextOptimizer 阶段被调用。
    const sadConfig = skillRoutingConfig.sad;
    const sadResult = await decomposeWithSAD(
      task,
      skills,
      {
        maxIterations: sadConfig.maxIterations,
        convergenceTau: sadConfig.convergenceTau,
        inputSideFeedback: sadConfig.inputSideFeedback,
      },
      decomposeFn,
      (subTask, availableSkills) => retrieveSkill(subTask, availableSkills),
    );
    subTasks = sadResult.subTasks;
    logger.info('compositional-router: SAD 分解完成', {
      iterations: sadResult.iterations,
      converged: sadResult.converged,
      subTaskCount: subTasks.length,
    });
  } else {
    // 未启用 SAD → 用原有 decomposeWithSkillAwareness
    subTasks = await decomposeWithSkillAwareness(task, skills, config, decomposeFn);
  }
  ...
  // 2. 粒度审计（仅记录问题，不强制修改分解结果）
  if (skillRoutingConfig.granularityAudit?.enabled) {
    const auditor = new DecompositionGranularityAuditor(skillRoutingConfig.granularityAudit);
    const issues = auditor.audit({ query: task, subTasks });
    if (issues.length > 0) {
      logger.info('compositional-router: 粒度审计发现问题', {
        count: issues.length,
  ...
  // 3. 检索匹配（仍用 retrieveSkill；bi-encoder 用于后续 contextOptimizer）
  const matches: SkillMatch[] = [];
  for (const sub of subTasks) {
    const match = retrieveSkill(sub, skills);
    if (match) matches.push(match);
  ...
  // 4. 组合 DAG
  let plan = composeDAG(matches, subTasks);

  // 5. 兼容性评分过滤
  if (skillRoutingConfig.compatibilityScorer?.enabled) {
    const scorer = new CompatibilityScorer(skillRoutingConfig.compatibilityScorer);
    const candidates = buildCandidateEdges(plan);
    const filteredEdges = scorer.filterEdges(candidates);
    ...
    plan = {
      nodes: plan.nodes,
      edges: newEdges,
      parallelGroups: topologicalSort({ nodes: plan.nodes, edges: newEdges, parallelGroups: [] }),
    };
    logger.info('compositional-router: 兼容性评分过滤完成', {
      before: candidates.length,
      after: filteredEdges.length,
    });
  }

  // 6. 上下文优化（实际调用 bi-encoder retriever）
  if (skillRoutingConfig.contextOptimizer?.enabled && biRetriever && biRetriever.isReady()) {
    const optimizer = new SkillContextOptimizer(
      biRetriever,
      skillRoutingConfig.contextOptimizer,
    );
    try {
      const optimized = await optimizer.buildContext(subTasks, skills);
      logger.info('compositional-router: 上下文优化完成', {
        skills: optimized.skills.length,
        estimatedTokens: optimized.estimatedTokens,
        truncated: optimized.truncated,
      });
    } catch (err) {
      logger.warn('compositional-router: 上下文优化失败', {
        error: err instanceof Error ? err.message : String(err),
      });
    }
  }
