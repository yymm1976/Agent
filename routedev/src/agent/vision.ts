...
export class VisionAssistant {
  ...
  static async loadImage(filePath: string, projectRoot?: string): Promise<ImageInput | null> {
    try {
      ...
      // Phase 29 Task 5：路径边界检查加固（修复 S12）
      // 原逻辑用 startsWith，存在前缀匹配绕过：
      //   allowedRoot=/project，/project-secret/file 也会通过（前缀匹配但非子目录）
      // 改用 path.relative：相对路径不以 .. 开头且不是绝对路径，才说明在 allowedRoot 内
