...
//   1. Sanitizer：安全检查 + 脱敏（ToolResultSanitizer）
//   2. Concise Thinking：> maxChars 字符时 800 首 + 标记 + 800 尾（trimToolResult）
//   3. Budget Offload：超长输出写文件 + 返回 preview（Task D7 实装）
//
// 设计要点：
//   - 零回归：process() 的输出与原 sanitizeToolResult 在同等配置下完全一致
//   - fail-open：sanitizer/offload 抛错时不阻断工具执行，降级到内存截断
...
export interface ToolOutputResult {
  ...
  /** 处理阶段记录（用于调试，如 ['sanitizer', 'concise-thinking', 'budget-offload']） */
  stages: string[];
}
 ...
 * 工具输出统一处理 pipeline
 *
 * 把 Sanitizer / Concise Thinking / Budget Offload 三个阶段收拢到 process() 方法，
...
export class ToolOutputPipeline {
  ...
  /**
   * 处理工具输出，依次执行：
   *   1. Sanitizer：安全检查 + 脱敏（fail-open：抛错时返回原内容）
   ...
   */
  process(toolName: string, result: string): ToolOutputResult {
    const stages: string[] = [];
    let processed = result;

    // 阶段 1：Sanitizer 安全检查 + 脱敏
