...
// PermissionChecker 单元测试（Phase 9 自主模式 API）
...
describe('PermissionChecker', () => {
  it('should return auto for unknown tools', () => {
    const checker = new PermissionChecker('semi');
  ...
  it('should auto-approve confirm in auto mode', () => {
  ...
  it('should return confirm in manual mode', () => {
    const checker = new PermissionChecker('manual');
    checker.addRule({ toolPattern: 'shell_exec', level: 'confirm' });
    expect(checker.checkPermission('shell_exec', {})).toBe('confirm');
  });

  it('should return confirm in semi mode', () => {
    const checker = new PermissionChecker('semi');
    checker.addRule({ toolPattern: 'shell_exec', level: 'confirm' });
    expect(checker.checkPermission('shell_exec', {})).toBe('confirm');
  });
  ...
  it('should honor autoApprovePatterns regardless of mode', () => {
    const checker = new PermissionChecker('manual', ['shell_exec']);
    checker.addRule({ toolPattern: 'shell_exec', level: 'confirm' });
    expect(checker.checkPermission('shell_exec', {})).toBe('auto');
  });
  ...
  it('should allow runtime mode change', () => {
    ...
    checker.addRule({ toolPattern: 'shell_exec', level: 'confirm' });
    expect(checker.checkPermission('shell_exec', {})).toBe('auto');
    checker.setAutonomyMode('manual');
    expect(checker.checkPermission('shell_exec', {})).toBe('confirm');
  });
});
